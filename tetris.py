...
brd_color, bg_color, txt_color, title_color, info_color = white, black, white, colors[3], colors[2]
...
def main():
    ...
    basic_font = pg.font.Font('12568.otf', 25)
    big_font = pg.font.Font('12568.otf', 75)
    pg.display.set_caption('Тетрис')
    showText('Тетрис')
    while True:  # начинаем игру
        runTetris()
        pauseScreen()
        showTextRed('Game over')
...
def showTextRed(text):
    titleSurf, titleRect = txtObjects(text, big_font, info_color)
    titleRect.center = (int(window_w / 2) - 3, int(window_h / 2) - 40)
    display_surf.blit(titleSurf, titleRect)

    pressKeySurf, pressKeyRect = txtObjects('Нажмите любую клавишу для продолжения', basic_font, info_color)
    pressKeyRect.center = (int(window_w / 2), int(window_h / 2) + 30)
    display_surf.blit(pressKeySurf, pressKeyRect)
...
def drawTitle():
    ...
    titleRect.topleft = (window_w - 405, 5)
    display_surf.blit(titleSurf, titleRect)
...
def drawInfo(points, level):
    ...
    pausebSurf = basic_font.render('space', True, title_color)
    pausebRect = pausebSurf.get_rect()
    pausebRect.topleft = (window_w - 77, 455)
    display_surf.blit(pausebSurf, pausebRect)

    pausebSurf = basic_font.render('Пауза:', True, txt_color)
    pausebRect = pausebSurf.get_rect()
    pausebRect.topleft = (window_w - 145, 455)
    display_surf.blit(pausebSurf, pausebRect)

    escbSurf = basic_font.render('Esc', True, info_color)
    escbRect = escbSurf.get_rect()
    escbRect.topleft = (window_w - 502, 455)
    display_surf.blit(escbSurf, escbRect)

    escbSurf = basic_font.render('Выход:', True, txt_color)
    escbRect = escbSurf.get_rect()
    escbRect.topleft = (window_w - 580, 455)
    display_surf.blit(escbSurf, escbRect)
...
def drawnextFig(fig):  # превью следующей фигуры
    ...
    nextRect.topleft = (window_w - 165, 180)
    display_surf.blit(nextSurf, nextRect)
    drawFig(fig, pixelx=window_w - 150, pixely=220)
